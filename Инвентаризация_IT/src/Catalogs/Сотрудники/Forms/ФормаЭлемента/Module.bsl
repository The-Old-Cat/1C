...
// Формируем текст для вывода
  ...
  КонецЕсли;
	...
	РабочееМесто = ПолучитьДанныеДокументаРабочиеМеста(ФИОСотрудника);
	
	// Создаем массив и добавляем значения
  Результат = Новый Массив;
  Результат.Добавить(ТекстДляВывода); 
...
Процедура УчетныеЗаписиПриИзменении(Элемент)
	...
	ТекстДляВывода = СформированныйТекст[0];
	РабочееМесто = СформированныйТекст[1];

	
	Элементы.СписокНеобходимыхДействий.Заголовок = ТекстДляВывода;
	Элементы.РабочееМесто.Заголовок = РабочееМесто;
КонецПроцедуры
